...
Downloading a single data set will result in a tibble whereas downloading multiple data sets will return a list containing each tibble. You can use `%<-%` (imported from the [zeallot package](https://github.com/r-lib/zeallot)) for unpacking a list with multiple tibbles to their own global environment object. For specifc details on a given data set see the data sets respective help file (i.e. `?transactions`).
